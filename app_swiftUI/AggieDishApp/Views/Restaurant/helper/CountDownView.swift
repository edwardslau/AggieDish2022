  ...
  var body: some View {
    Circle()
      ...
      .overlay(
        Circle()
          .stroke(
            (isLessthanFiveMin(countTo: countTo, counter: counter) ? Color.green : Color.red),
            lineWidth: 12)
      )
  }
}
    ...
    Circle()
      ...
      .overlay(
        Circle().trim(from: 0, to: progress(countTo: countTo, counter: counter))
          ...
          .foregroundColor(
            (isLessthanFiveMin(countTo: countTo, counter: counter)
              ? (completed(countTo: countTo, counter: counter) ? Color.green : lightGreen)
              : lightRed)
          )
